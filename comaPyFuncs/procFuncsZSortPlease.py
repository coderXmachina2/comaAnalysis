...
def smooth_ellipse_edge(
    imagein,
    ellipseParamIn,  # (Center, tradiparams)... unfortunately you have to give a new one
    sigma=3,
    edge_width=4,
    plotverbose=True,
):
    """
    Gaussian smooth only along the edge of an elliptical region.

    Parameters:
    - image: 2D array, the input image to be processed
    - elllipseParamIn: ((y, x), (sma, ellipticity), theta )
    - theta: float, the rotation angle of the ellipse in radians
    - sigma: float, the standard deviation of the Gaussian filter
    - edge_width: int, the width of the edge region to smooth

    Returns:
    - smoothed_image: 2D array, the image with smoothing applied only along the ellipse edge
    """
    geometry = EllipseGeometry(
        x0=ellipseParamIn[0][1],
        y0=ellipseParamIn[0][0],
        sma=ellipseParamIn[1][0],
        eps=ellipseParamIn[1][1],  # What is an ellipse eps
        pa=ellipseParamIn[1][2] * np.pi / 180.0,
    )

    # This is just for aperture position
    ...
    aperture_mask = ellipse_aperture.to_mask().to_image(imagein.shape)

    # Create a border mask
    border_mask = np.zeros_like(aperture_mask, dtype=bool)

    # Create a dilation mask to identify border pixels
    structure = np.ones((2 * edge_width + 1, 2 * edge_width + 1), dtype=bool)
    dilated_mask = ndi.binary_dilation(aperture_mask, structure=structure)
    border_mask = dilated_mask & np.logical_not(aperture_mask)

    # Apply Gaussian filter to the entire image
    smoothed_image = ndi.gaussian_filter(imagein, sigma=sigma)

    # Smooth only along the border
    smoothed_edge_image = imagein.copy()
    smoothed_edge_image[border_mask] = smoothed_image[border_mask]

    if plotverbose:
        plt.figure(figsize=(16, 8))
        plt.imshow(imagein, origin="upper")

        plt.grid(alpha=0.2)
        plt.title("Aperture")

        plt.colorbar()
        levels = np.linspace(np.min(imagein), np.max(imagein), num=7)
        ...
        ellipse_aperture.plot(color="gold", ls="--")
        plt.show()

    return smoothed_edge_image


def fit_Ellipse(imagein, paramsEllipse):
    elipt = 1 - (paramsEllipse[1][1] / paramsEllipse[1][0])

    geometry = EllipseGeometry(
        x0=paramsEllipse[0][1],
        y0=paramsEllipse[0][0],
        sma=paramsEllipse[1],
        eps=elipt,  # What is an ellipse eps
        pa=paramsEllipse[2] * np.pi / 180.0,
    )
    ...
    ellipse = Ellipse(imagein, geometry)

    isolist = ellipse.fit_image()
    model_image = build_ellipse_model(imagein.shape, isolist)
...
def fit_ellipse_params(list_params):
    ...
    parmsSemiMaj, covarsemiMaj = curve_fit(logis_func, x_data, list_params, p0=igdata)
    L, k, x0 = parmsSemiMaj
...
def process_derivative(inDeriv, threshF=0.01, verboseH=False):
    ...
    false_colour = np.full_like(
        Ix, fill_value=1000
    )  # Assuming 255 is the default value for other points

    # Find locations where Ix and Iy are effectively zero (x, y)
    neutral_points = np.where(
        (np.abs(Ix) <= Ix_threshold) & (np.abs(Iy) <= Iy_threshold)
    )
    ...
    if verboseH:
        ...
        print(
            "Ix value:",
            Ix[neutral_points[0][0]][neutral_points[1][0]],
            "of y:",
            neutral_points[0][0],
            "x:",
            neutral_points[1][0],
        ...
        print(
            "Iy value:",
            Iy[neutral_points[0][0]][neutral_points[1][0]],
            "of y:",
            neutral_points[0][0],
            "x:",
            neutral_points[1][0],
        ...
        print(
            "Ix value:",
            Ix[neutral_points[0][1]][neutral_points[1][1]],
            "of y:",
            neutral_points[0][1],
            "x:",
            neutral_points[1][1],
        ...
        print(
            "Iy value:",
            Iy[neutral_points[0]][1][neutral_points[1][1]],
            "of y:",
            neutral_points[0][1],
            "x:",
            neutral_points[1][1],
        ...
        print(
            "Ix value:",
            Ix[neutral_points[0][2]][neutral_points[1][2]],
            "of y:",
            neutral_points[0][2],
            "x:",
            neutral_points[1][2],
        ...
        print(
            "Iy value:",
            Iy[neutral_points[0][2]][neutral_points[1][2]],
            "of y:",
            neutral_points[0][2],
            "x:",
            neutral_points[1][1],
    ...
    if verboseH:
        print("Sample:")
        print(
            "\nOnly Ix is less than 0:",
            Ix[only_Ix_zero[0][0]][only_Ix_zero[1][0]],
            "<",
            Ix_threshold,
            "<",
            Iy[only_Ix_zero[0][0]][only_Ix_zero[1][0]],
        )
        print(
            "Only Iy is less than 0:",
            Iy[only_Iy_zero[0][0]][only_Iy_zero[1][0]],
            "<",
            Iy_threshold,
            "<",
            Ix[only_Iy_zero[0][0]][only_Iy_zero[1][0]],
        )
        # Implement for loop
...
def present_search(a_dict, verboseH=False):
    ...
    if verboseH:
        print("N neutral points:          ", len(a_dict["NeutralPoints"]))
        print("Determinants > 0:          ", len(a_dict["Dets>0"]))
        print(
            "Intersect neutrals & dets: ",
            len(coordsetintersect(a_dict["NeutralPoints"], a_dict["Dets>0"])),
        )
...
def filter_coords(list_a, xlims=(), ylims=(), verboseH=False):
    ...
    if len(xlims) > 0 and len(ylims) > 0:
        for coordset in list_a:
            if (
                coordset[0] > ylims[0]
                and coordset[0] < ylims[1]
                and coordset[1] > xlims[0]
                and coordset[1] < xlims[1]
            ):
...
def Combine(indict, jointup, verboseC=False):
    ...
    if verboseC:
        ...
        print(
            "Last of reversed:", np.array([x[0] for x in indict[jointup[0]]])[::-1][-1]
        )
        print(
            "First of right side:", np.array([x[0] for x in indict[jointup[1]]])[::][0]
        )

    joined = np.concatenate(
        (
            np.array([x[0] for x in indict[jointup[0]]])[::-1][
                :-1
            ],  # Flipped and trimmed. Why do you trim it like that? [:-1]
            np.array([x[0] for x in indict[jointup[1]]])[::][:],
        ),
        axis=0,
    )  # [:-1] #Everything until the last one...

    if verboseC:
        print(
            "Vals at start (verify): ", [indict[jointup[0]][0], indict[jointup[1]][0]]
        )  # This is already proof...
        print("Joining: ", jointup[0], " with ", jointup[1])
        print(
            "Length of " + jointup[0] + " reversed and pruned: ",
            len(np.array([x[0] for x in indict[jointup[0]]])[::-1][1:]),
        )
        print(
            "Length of " + jointup[1] + ": ",
            len(np.array([x[0] for x in indict[jointup[1]]])[::][:]),
        )
        print(
            "Final Length:",
            len(np.array([x[0] for x in indict[jointup[0]]])[::-1][:-1])
    ...
    return [joined, len(np.array([x[0] for x in indict[jointup[0]]])[::-1][1:])]
...
def sample_in_directions(image, cent_loc, verboseH=False, verboseL=False):
    ...
    if verboseH:
        print(
            "Amplitude at sampled center (Cartesian) (",
            cent_loc[1],
            cent_loc[0],
            ")",
            image[cent_loc[0]][cent_loc[1]],
        )
        print()
...
def rectify_walk(walkdata, lens, joinslist, verboseH=False, verboseL=False):
    ...
    for k in range(0, len(walkdata)):  # in datasjoined:
        if verboseL:
            ...
            print("")
            print(
                "Anchor verify:", walkdata[k][lens[k]], "index:", lens[k]
            )  # Lat value...
            print()
            print(
                "Max of data:",
                np.max(walkdata[k]),
                "index:",
                np.where(walkdata[k] == np.max(walkdata[k])),
            )
            print("")
            print("----")
            print("")
    ...
    if verboseH:
        ...
        for k in range(0, len(legendhold)):
            print(
                legendhold[k],
                ":",
                [(np.max(datalens) - x) for x in datalens][k],
                "spaces",
            )
