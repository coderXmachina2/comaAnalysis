...
def hessianStats(image):
    ...
    Ix = ndimage.sobel(
        image, axis=0, mode="constant", cval=0.0
    )  # This is a first derivative
    Iy = ndimage.sobel(
        image, axis=1, mode="constant", cval=0.0
    )  # This is a first derivative
    ...
    Iyx = ndimage.sobel(
        Iy, axis=0, mode="constant", cval=0.0
    )  # Cross-derivative... The only true second order
    ...
    Hessian = np.array([[Ixx, Ixy], [Iyx, Iyy]])
