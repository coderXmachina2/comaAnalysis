...
def plotCompare2Lines(listData, listLegend, labels=[], suptitle="", noise=False):
    """
    ...
    if isinstance(listData[0], tuple):
        scatter_major = plt.scatter(listData[0][0], listData[0][1], label=listLegend[0])
        line_major = plt.plot(listData[0][0], listData[0][1])
    else:
        scatter_major = plt.scatter(
            np.arange(0, len(listData[0])), listData[0], label=listLegend[0]
        )
        line_major = plt.plot(np.arange(0, len(listData[0])), listData[0])

    # If the first is tuple
    if isinstance(listData[1], tuple):
        scatter_minor = plt.scatter(listData[1][0], listData[1][1], label=listLegend[1])
        line_minor = plt.plot(listData[1][0], listData[1][1])
    else:
        # Plot the semi-minor axis
        scatter_minor = plt.scatter(
            np.arange(0, len(listData[1])), listData[1], label=listLegend[1]
        )
        line_minor = plt.plot(np.arange(0, len(listData[1])), listData[1])

    # Create custom legend handles
    custom_major = Line2D(
        [0], [0], color="#1f77b4", marker="o", linestyle="-", markersize=8
    )
    custom_minor = Line2D(
        [0], [0], color="#ff7f0e", marker="o", linestyle="-", markersize=8
    )

    if noise:
        noise_plt = plt.plot(listData[-1], label="Bkg noise")
        custom_noise = Line2D(
            [0], [0], color="#2ca02c", marker="", linestyle="-", markersize=8
        )
    plt.title(suptitle)
    plt.xlabel(labels[0])
    plt.ylabel(labels[1])
    ...
    if noise:
        plt.legend(
            [custom_major, custom_minor, custom_noise],
            [listLegend[0], listLegend[1], listLegend[-1]],
        )
    else:
...
def plot_4subp1D_spectra(
    spectra, subtitles, suptitle, highlightzero=False, lesiterables=False
):
    """
