...
def centfindpipe(
    ...
    upScaledNeutrals = procFuncsZSortPlease.process_derivative(
        img_Hessian["FirstOrder"], threshF=zin, verboseH=verbosityF
    )

    coordsetDet = procFuncsZSortPlease.process_determinant(
        img_Hessian["DeterminantHessian"],
        "Determinant",
        search="min",
        verboseH=verbosityF,
    )
    coordsetmax = procFuncsZSortPlease.process_determinant(
        img_Hessian["Hessian"][0][0], "Ixx", search="max", verboseH=verbosityF
    )  # Greater than 0

    adict = {
        "NeutralPoints": upScaledNeutrals[1],
        "Dets>0": coordsetDet,
        "Ixx<0": coordsetmax,
    }

    localmax, neutralmaxs = procFuncsZSortPlease.present_search(
        adict, verboseH=verbosityF
    )
    inner = procFuncsZSortPlease.filter_coords(
        neutralmaxs, xlims=innerargs[0], ylims=innerargs[1], verboseH=verbosityF
    )
    ...
    if verbosityT:
        ...
        print(
            "Cartesian (x, y): (",
            int(np.mean([x[1] for x in inner])),
            ",",
            int(np.mean([x[0] for x in inner])),
            ")",
        )
        print(
            "Computational [depth][width]: [",
            int(np.mean([x[0] for x in inner])),
            "][",
            int(np.mean([x[1] for x in inner])),
            "]",
    ...
    if plotTrue:
        plotFuncsIm.plot_1_im(
            image=procFuncsZSortPlease.set_pixels(inmage, inner),  # , sub_val=0.00001),
            header="",
            title=imtype
            + " Filtered Center. N local max neutral Points discovered: "
            + str(len(inner)),
            ...
            targeter=[
                (
                    int(np.mean([x[1] for x in inner])),
                    1000 - int(np.mean([x[0] for x in inner])),
                )
            ],  # targeter is a scatter point
            contour=True,
        )
...
def Fsmoothing(inmage, zeroutparams, newSize, verboseT=False):
    ...
    zerout = procFuncsZSortPlease.zero_outside_radius(
        procFuncsZSortPlease.FFThings(inmage)["fftshiftfimage"],
        (zeroutparams[0][0], zeroutparams[0][1]),
        zeroutparams[1],
    )
