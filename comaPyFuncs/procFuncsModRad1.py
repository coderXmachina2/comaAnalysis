...
def mask_annuli(
    ...
    if mtype == "sphere":
        inner_pixel_values, inner_circle_coords = extract_pixels_within_circle(
            image, center, radii[0]
        )
        ...
        if verboseDebug:
            ...
            print(
                "Inner inner pixel values:",
                inner_pixel_values,
                "This test->",
                np.mean(inner_pixel_values),
                "Test val fin ->",
                (np.mean(inner_pixel_values) * coeff[0]) + coeff[1],
            )
            print("Inner inner circle coordinates", inner_circle_coords)
    elif mtype == "ellipse":
        inner_pixel_values, inner_circle_coords = extract_pixels_within_ellipse(
            image, center, ellipse=(radii[0])
        )
        if verboseDebug:
    ...
    if inner_pixel_values:
        if len(coeff) > 0:
            ...
            if computation == "mean":
                inner_average_value = (
                    np.mean(inner_pixel_values) * (coeff[0])
                ) + coeff[1]
                if verboseDebug:
                    print(
                        "Inner average value: ->", inner_average_value
                    )  # This value is printed but notadded
            elif computation == "median":
                inner_average_value = (
                    np.median(inner_pixel_values) * (coeff[0])
                ) + coeff[1]
                if verboseDebug:
    ...
    for i in range(
        len(radii) - 1
    ):  # This works with the circular model right... So the
        if mtype == "sphere":
            annulus_coords = find_annulus_coordinates_spec(
                masked_image, center, radii[i], radii[i + 1]
            )

        elif mtype == "ellipse":
            #### This goes into that section ###
            pixelsA, coordsetA = extract_pixels_within_ellipse(
                image, center, ellipse=radii[i]
            )
            pixelsB, coordsetB = extract_pixels_within_ellipse(
                image, center, ellipse=radii[i + 1]
            )
    ...
    if not bkg:
        if mtype == "sphere":
            ...
            for x in range(height):
                for y in range(width):
                    if not procFuncsMisc.is_within_ellipse(
                        x, y, center, radii[-1][1], radii[-1][0], radii[-1][2]
                    ):
                        masked_image[x, y] = 0
...
def find_annulus_coordinates_spec_GPT(image, center, inner_params, outer_params):
    ...
    for x in range(int(center[0] - outer_a), int(center[0] + outer_a + 1)):
        for y in range(int(center[1] - outer_b), int(center[1] + outer_b + 1)):
            if is_within_ellipse(
                x, y, center, outer_a, outer_b, outer_theta
            ) and not is_within_ellipse(x, y, center, inner_a, inner_b, inner_theta):
...
def find_annulus_coordinates_spec(image, center, inner_radius, outer_radius):
    """
    ...
    return [
        (int(x), int(y))
        for x in range(int(center[0] - outer_radius), int(center[0] + outer_radius + 1))
        for y in range(int(center[1] - outer_radius), int(center[1] + outer_radius + 1))
        if int(inner_radius**2)
        < (int(x - center[0]) ** 2 + (y - center[1]) ** 2)
        <= int(outer_radius**2)
    ]


def find_annulus_coordinates_gen(outer_circle_salty_coords, inner_circle_coords):
    """
    ...
    annulus_coordinates = [
        coord for coord in outer_circle_salty_coords if coord not in inner_circle_coords
    ]

    return annulus_coordinates


def mask_coordinates_with_specific_average(image, coordinates, computation="mean"):
    """
...
def extract_pixels_within_circle(image, center, radius):
    """
    ...
    if radius > 0:
        ...
        for y in range(int(min_y), int(max_y)):
            for x in range(int(min_x), int(max_x)):
                ...
                if (x - center[0]) ** 2 + (
                    y - center[1]
                ) ** 2 <= radius**2:  # DIstance with respect to center...
...
def extract_pixels_within_ellipse(image, center, ellipse=()):
...
def extract_circle_and_ellipse(
