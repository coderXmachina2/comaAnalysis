...
def convert_to_equatorial(header):
    ...
    if wcs_obj.wcs.lngtyp != 'RA':
        ...
        wcs_obj.wcs.set()
        wcs_obj.wcs.ctype = ['RA---TAN', 
                             'DEC--TAN']
    return wcs_obj.to_header()

def sliding_difference(data1,     #coma y map
                       d1blurred, #smooth model
                       max_depth):
    """
    ...
    for L in range(1, max_depth + 1):
        for dx in range(-L, L+1):
            for dy in range(-L, L+1):
                if np.abs(dx) == L or np.abs(dy) == L:  # Check if at the boundary of depth L
        ...
        for dx in range(-L, L+1):
            for dy in range(-L, L+1):
                if np.abs(dx) == L or np.abs(dy) == L:
                    ...
                    shifted_d1blurred[max(0, -dx):min(width, width - dx),
