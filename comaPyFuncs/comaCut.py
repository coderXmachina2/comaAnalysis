...
if not os.path.exists(outdir + "/hp_proj"):
    os.mkdir(outdir + "/hp_proj")
if not os.path.exists(outdir + "/drizzlib"):
    os.mkdir(outdir + "/drizzlib")

output_gal = True
if output_gal:
    map_proj = ["GLON-SIN", "GLAT-SIN"]
    which_coord = "G"
    switch_coord = ["G"]
else:
    map_proj = ["RA---TAN", "DEC--TAN"]
    which_coord = "C"
    switch_coord = ["G", "C"]
...
for ims, mapset in enumerate(mapsets):
    ...
    if hp.get_nside(ymap) != nside:
        print(
            "Warning: specified nside (%d) not consistent with nside from healpix map (%d)"
            % (nside, hp.get_nside(ymap))
        )
    nside = hp.get_nside(ymap)
    pix = hp.nside2resol(nside) * 180.0 / np.pi
    try:
    ...
    cen = [
        np.pi / 2.0 - patch_dec * np.pi / 180.0,
        patch_RA * np.pi / 180.0,
    ]  # (theta,phi) in radians

    r = hp.Rotator(coord=["C", "G"])
    cen_g = r(cen)
    patch_cen_g = [cen_g[1] * 180.0 / np.pi, 90 - cen_g[0] * 180.0 / np.pi]
    ...
    if output_gal:
        ...
        mapout2 = "%s/drizzlib/map%d_MILCA_Coma_20deg_G_%s.fits" % (
            outdir,
            nside,
            mapset,
        )
    ...
    y_patch2 = healpix2wcs.healpix2wcs(
        y_mapname,
        col_ids=[ims],
        header=mapout,
        header_hdu=1,
