...
def autocovariance_2d_a(image):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            autocov_matrix[i, j] = np.mean(
                image_zero_mean * shifted_image
            )  # Taking the mean of the product of the 0 mean image and the shifted
...
def img_diagnosat(image, integarround=10, verbose=False):
    """
    Returns statistics
    """
    ...
    if verbose:
        print(
            "Max:",
            np.round(np.max(image), 10),
            "Min:",
            np.round(np.min(image), 10),
            "Range:",
            np.round(np.max(image) + np.min(image), 10),
        )
        print(
            "Mean \u00B1 std:",
            np.round(np.mean(image), 10),
            "\u00B1",
            np.round(np.std(image), 10),
        )
        print(
            "Median \u00B1 median absolute deviation:",
            np.round(np.median(image), 10),
            "\u00B1",
            np.round(robust.mad(image.ravel()), 10),
        )
        print("Variance:", np.round(np.std(image) * np.std(image), 10))

    return {
        "Max": np.round(np.max(image), 10),
