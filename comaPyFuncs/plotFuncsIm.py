...
def plot_1_im(image='', 
    ...
    if(contour):
        ...
        levels = np.linspace(np.min(image), 
                             np.max(image), 
                             num=7)
        
        contours = ax.contour(image, levels,
                              colors='red', origin='upper', alpha=contourAlpha,
                              extent=[0, image.shape[1], 0, image.shape[0]])
    ...
    if len(spheres[1]) > 0:
        for idx, rads in enumerate(spheres[1]):
            centex, centey = int(image.shape[0]/2) - spheres[0][0], int(image.shape[1]/2) - spheres[0][1]
            ...
            if idx == len(spheres[1]) - 1:
                ax.plot([centex - rads, centex + rads], [centey, centey], 
                        color='red', linestyle='--', linewidth=1.5, alpha=masterAlpha)
                ax.plot([centex, centex], [centey - rads, centey + rads], 
                        color='red', linestyle='--', linewidth=1.5, alpha=masterAlpha)

    if len(ellipses[1])>0:
        for ellist in ellipses[1]:
            centex, centey = (image.shape[0]/2)-ellipses[0][0], (image.shape[1]/2)-ellipses[0][1]
    ...
    if len(targeter)>0:
        ...
        #Instead of using this use a 'plot'
            ...
            plt.plot([image.shape[0]/2, targeter[i][0]],
...
def plot_2_im_row(images, spheres=[], ellipses=[], targeter=[], subTitles=[], superTitles='', cmapN='',contour=False, gridl=False):
    """
    ...
    if (contour):
        levels = np.linspace(np.min(images[0]), 
                             np.max(images[0]), 
                             num=7)
        
        # Create contour lines over the image
        contours = axes[0].contour(images[0], levels,
                                   colors='red', origin='lower', alpha=0.32,
                                   extent=[0, images[0].shape[1], 0, images[0].shape[0]])
        # Add labels to the contours
        axes[0].clabel(contours, 
                       inline=True, 
                       fontsize=8, 
                       fmt='%.2e')
    ...
    if len(spheres)>0: #If not empty list
        ...
        centex, centey = spheres[0]['center'][1], spheres[0]['center'][0] #This goes to the ce
        ...
        axes[0].plot([centex - rads, centex + rads], [centey, centey], 
                     color='red', linestyle='--', linewidth=1.5, alpha=masterAlpha)
        axes[0].plot([centex, centex], [centey - rads, centey + rads], 
                     color='red', linestyle='--', linewidth=1.5, alpha=masterAlpha)
    """
    ...
    fig.colorbar(im2, ax=axes[1],
                 fraction=0.046, pad=0.04)  # Add colorbar
    axes[1].grid(alpha=0.2)

    # Generate contour levels
    if (contour):
    # Generate contour levels
        levels = np.linspace(np.min(images[1]), 
                             np.max(images[1]), 
                             num=7)
        
        # Create contour lines over the image
        contours = axes[1].contour(images[1], levels,
                              colors='red', origin='lower', alpha=0.32,
                              extent=[0, images[1].shape[1], 0, images[1].shape[0]])
        # Add labels to the contours
        axes[1].clabel(contours, 
                       inline=True, 
                       fontsize=8, 
                       fmt='%.2e')
    """
    ...
    if len(ellipses)>0: #If not empty list
        ...
        axes[1].plot([centex - end_major_x, centex + end_major_x], 
                     [centey - end_major_y, centey + end_major_y], 
                     color='red', linestyle='-.', linewidth=1.5, alpha=0.16)
    
                # Plot the semi-minor axis
        axes[1].plot([centex - end_minor_x, centex + end_minor_x], 
                     [centey - end_minor_y, centey + end_minor_y], 
                     color='red', linestyle='--', linewidth=1.5, alpha=0.16)
    ...
    if len(targeter)>0:
        for i in range(0, len(targeter)):
            ...
            axes[0].scatter(targeter[i][0],   targeter[i][1],  marker=(5, 2), color = scatterc)
            axes[0].plot([images[0].shape[0]/2, targeter[i][0]],
                     [images[0].shape[1]/2, targeter[i][1]],
                     linestyle='--', color=scatterc)

            axes[1].scatter(targeter[i][0],   targeter[i][1],  marker=(5, 2), color = scatterc)
            axes[1].plot([images[0].shape[0]/2, targeter[i][0]],
                     [images[0].shape[1]/2, targeter[i][1]],
                     linestyle='--', color=scatterc)
...
def plot_3_im_row(images, 
    ...
    if len(images) != 3 or len(subTitles) != 3 or len(headers) != 3:
        raise ValueError("Images, headers, and titles lists must each contain exactly three elements.")
    ...
    for i, ax in enumerate(axs):
        ...
        im = ax.imshow(images[i], 
                       aspect='auto', 
                       cmap='viridis', 
                       norm=Normalize(vmin=np.min(images[i]), 
        ...
        # Generate contour levels
            ...
            # Create contour lines over the image
            contours = axes.contour(images[1], levels,
                                  colors='red', origin='upper', alpha=0.32,
                                  extent=[0, images[1].shape[1], 0, images[1].shape[0]])
            # Add labels to the contours
        ...
        ax.grid(True, 
                which='major',
                color='white', linestyle='--', linewidth=0.25, alpha=0.35)
        
    plt.tight_layout()
    plt.show()

def plot_img_and_diag4x4(image, 
                         header, 
                         inbins=256, 
                         supT='', 
                         circrad=0,
    ...
    ax2.text(0, np.mean(image.ravel()), "mean:"+str(np.round(np.mean(image.ravel()), 8)), color='red', fontsize=16)
    ...
    if circrad != 0:
        ...
        circle = Circle((center_x, center_y), 
                         ...
                         linewidth=1.5,
                         alpha=0.55)
        circpixes = extract_pixels_within_circle(image, (int(center_x), int(center_y)) , circrad )
